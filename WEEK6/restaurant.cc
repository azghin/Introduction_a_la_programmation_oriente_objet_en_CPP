...
public:
    ...
    {
        Ingredient* in = new Ingredient(p, quantite);
        li.push_back(in);
    }
    ...
    Recette adapter(double n) const
    {
        ...
        {
            r.ajouter(li[i]->getProduit(), li[i]->getQuantite() * n);
        }
        return r;
    }
    ...
    string toString() const
    {
        ...
        {
            if (i == li.size() - 1)
                s += to_string(i + 1) + ". " + li[i]->descriptionAdaptee();
            else
                s += to_string(i + 1) + ". " + li[i]->descriptionAdaptee() + "\n";
        }
        return s;
    ...
    virtual double quantiteTotale() const
    {
        double sum(0);
        for (int i = 0; i < li.size(); i++)
            sum += li[i]->getQuantite();
        return sum;
    }
};
...
void afficherQuantiteTotale(const Recette& recette, const Produit& produit)
{
  ...
  cout << "Cette recette contient " << recette.quantiteTotale()
